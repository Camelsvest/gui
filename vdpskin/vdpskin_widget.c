...
static void vdpskin_drawFocusFrame(mWidget *self, HDC hdc, const RECT* rc)
{
	ncsCommRDRDrawFocusFrame(self->hwnd, hdc, rc);
}

static BOOL vdpskin_drawBkground(mWidget *self, HDC hdc, const RECT* inv_rc)
{
	return ncsCommRDRDrawBkgnd(self->hwnd, hdc, inv_rc);
}

static void vdpskin_draw3dbox(mWidget *self, HDC hdc, const RECT *rc, DWORD color, DWORD flag)
{
	ncsCommRDRDraw3dbox(hdc, rc, color, flag);
}

static void vdpskin_drawCheckbox(mWidget *self, HDC hdc, const RECT *rc, DWORD flag)
{
...
static void vdpskin_drawRadio(mWidget *self, HDC hdc, const RECT *rc, DWORD flag)
{
	DWORD fgcolor = ncsGetElement(self, NCS_FGC_WINDOW);
	DWORD bgcolor = ncsGetElement(self, NCS_BGC_WINDOW);
	DWORD discolor = ncsGetElement(self, NCS_FGC_DISABLED_ITEM);
...
static void vdpskin_drawArrow(mWidget *self, HDC hdc, 
...
static void vdpskin_drawItem(mWidget *self, HDC hdc, const RECT *rc, DWORD flag)
{
	int id;
	DWORD color;
...
static void vdpskin_class_init(mWidgetRenderer * rdr)
{
    rdr->rdr_name       = "vdpskin";
	rdr->drawFocusFrame = vdpskin_drawFocusFrame;
	rdr->drawBkground   = vdpskin_drawBkground;
	rdr->draw3dbox      = vdpskin_draw3dbox;
	rdr->drawCheckbox   = vdpskin_drawCheckbox;
	rdr->drawRadio      = vdpskin_drawRadio;
	rdr->drawArrow      = vdpskin_drawArrow;
	rdr->drawItem       = vdpskin_drawItem;
}

mWidgetRenderer vdpskin_widget_renderer = {
    "",
	vdpskin_class_init,
	NULL,
};
