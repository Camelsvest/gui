...
extern mWidgetRenderer vdpskin_widget_renderer;

static void vdpskin_resetHeadArea (mPropSheet *self, RECT* rcClient, DWORD style)
{
...
static void vdpskin_getRect(mPropSheet *self,
...
static void vdpskin_drawBorder(mPropSheet *self, HDC hdc, RECT* rcBorder)
{
...
static void vdpskin_drawScrollBtn(mPropSheet *self, HDC hdc,
        RECT* rcBtn, int which)
{
	self->renderer->drawArrow(self, hdc, rcBtn, which, 0L, TRUE);
}

static void vdpskin_drawTab(mPropSheet *self, HDC hdc,
...
static void vdpskin_propsheet_class_init (mPropSheetRenderer* rdr)
{
	vdpskin_widget_renderer.class_init((mWidgetRenderer*)rdr);
	rdr->resetHeadArea = vdpskin_resetHeadArea;
	rdr->getRect       = vdpskin_getRect;
	rdr->drawScrollBtn = vdpskin_drawScrollBtn;
	rdr->drawBorder    = vdpskin_drawBorder;
	rdr->drawTab       = vdpskin_drawTab;
}

mPropSheetRenderer vdpskin_propsheet_renderer = {
    "",
	vdpskin_propsheet_class_init,
	&vdpskin_widget_renderer,
};
